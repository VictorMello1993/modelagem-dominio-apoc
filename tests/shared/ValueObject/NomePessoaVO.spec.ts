...
test("Deve lançar erro ao tentar criar nome vazio", () => {
  expect(() => new NomePessoaVO("")).toThrow(ErrosPessoa.NOME_VAZIO);
});

test("Deve lançar erro ao tentar criar nome menor que 4 caracteres", () => {
  expect(() => new NomePessoaVO("ab c")).toThrow(ErrosPessoa.NOME_PEQUENO);
});
...
test("Deve lançar erro ao tentar criar nome maior que 120 caracteres", () => {
  ...
  expect(() => new NomePessoaVO(nome)).toThrow(ErrosPessoa.NOME_GRANDE);
});

test("Deve lançar erro ao tentar criar nome sem sobrenome", () => {
  expect(() => new NomePessoaVO("Victor")).toThrow(ErrosPessoa.NOME_SEM_SOBRENOME);
});

test("Deve lançar erro ao tentar criar nome com caracteres especiais", () => {
  expect(() => new NomePessoaVO("Victor Santos @0000Victor")).toThrow(ErrosPessoa.NOME_CARACTERES_INVALIDOS);
});
